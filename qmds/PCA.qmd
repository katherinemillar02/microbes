...
<strong>Why do we need to use PCA with huge microbial datasets?</strong>

<form>
  <select id="q1">
    <option value="">-- Select an answer --</option>
    <option value="a">A. To increase the number of variables in a dataset</option>
    <option value="b">B. To reduce dimensionality by summarizing correlated variables while retaining most of the variance</option>
    <option value="c">C. To normalize data by rescaling variables to the same units</option>
    <option value="d">D. To rank variables according to their original measurement units</option>
  </select>
  <br><br>

  <button type="button" onclick="checkq1()">Submit</button>
</form>

<div id="result1" style="margin-top: 1em; font-weight: bold;"></div>
...
function checkq1() {
  ...
  if (answer === "b") {
    result.textContent =
      "✅ Correct! PCA reduces dimensionality by capturing most of the variance in fewer uncorrelated components.";
    result.style.color = "green";
  } else {
    result.textContent =
      "❌ Not quite. Think about dimensionality reduction and correlated variables.";
...
PCA assumes that the main patterns in the data can be described using linear (straight-line) relationships. It identifies new axes that are linear combinations of the original variables and captures variation along these straight-line directions. As a result, PCA works best when relationships between variables are approximately linear, and it may fail to represent complex non-linear patterns in the data.

<strong>What measures distances in PCA, and why?</strong>

<select id="pca-answer">
  <option value="">-- Select an option --</option>

  <option value="a">
    A. Manhattan distance, because PCA minimizes absolute differences between variables
  </option>

  <option value="b">
    B. Euclidean distance in the original variable space, because PCA preserves raw feature distances
  </option>

  <option value="c">
    C. Euclidean distance in principal component space, because PCA projects data onto orthogonal axes that capture variance
  </option>

  <option value="d">
    D. Bray–Curtis dissimilarity, because PCA is designed for compositional count data
  </option>
</select>
<br><br>
...
function checkAnswer() {
  const answer = document.getElementById("pca-answer").value;
  ...
  if (answer === "c") {
    result.textContent =
      "✅ Correct! Distances in PCA plots are Euclidean distances in principal component space, where samples are projected onto orthogonal axes that capture the greatest variance.";
    result.style.color = "green";
  } else {
    result.textContent =
      "❌ Not quite. Think about how PCA projects data onto new orthogonal axes and how distances are interpreted in that space.";
    result.style.color = "red";
  }
}
</script>
...
-   A covariance close to zero suggests little or no linear
    ...
    <strong>Why do we need the covariance matrix?</strong>

<form>
  <select id="q3">
    <option value="">-- Select an answer --</option>
    <option value="a">A. To see how our values vary and go in directions together. </option>
    <option value="b">B. To ensure our values are integers. </option>
    <option value="c">C. To make the average across samples 0. </option>
    <option value="d">D. Because there needs to centered data. </option>
  </select>

  <button type="button" onclick="checkPCA1()">Submit</button>
</form>
...
<button onclick="toggleVisibility(&#39;centered-data-container&#39;)">Show/Hide `centered_data_matrix <- as.matrix(centered_data)` output</button>
...
<button onclick="toggleVisibility(&#39;cov-matrix-output&#39;)">Show/Hide Manual R `cov_matrix_1` output</button>
...
Show/Hide R Function `cov_matrix_R` output </button>
...
::: {#quizs1}
<form id="quiz-form-s1">

<input type="radio" name="sq1" value="a"> A. 1. Center the data, 2. Scale
...
<button type="button" onclick="checkAnswers1()">Submit
</button></form>

::: {#results1 style="margin-top: 1em; font-weight: bold;"}
:::
:::
...
function checkAnswers1() {
  const answer = document.querySelector('#quiz-form-s1 input[name="sq1"]:checked');
...
<button type="button" onclick="checkAnswer2()">Submit</button></form>
...
<div id="quizs3">
<form id="quiz-form-s3">

<input type="radio" name="sq3" value="a">
A. Eigenvalues determine the correlation between original variables; eigenvectors show how to cluster them<br>

<input type="radio" name="sq3" value="b">
B. Eigenvalues represent the amount of variance captured by each principal component; eigenvectors define the direction of these components<br>

<input type="radio" name="sq3" value="c">
C. Eigenvalues are used to normalize the data; eigenvectors eliminate redundancy<br>

<input type="radio" name="sq3" value="d">
D. Eigenvalues and eigenvectors sort features in descending order of importance based on their original units<br><br>

<button type="button" onclick="checkAnswers3()">Submit</button>

</form>

<div id="results3" style="margin-top: 1em; font-weight: bold;"></div>
</div>

```{=html}
<script>
function checkAnswers3() {
  const answer = document.querySelector('#quiz-form-s3 input[name="sq3"]:checked');
...
<p>Cool stuff. You have now completed learning about Principal Component Analysis, please click
the button below to begin learning about Principal Co-ordinate Analysis!</p>

::: {style="text-align: right; margin-top: 2em;"}
<a href="PCoA.qmd" class="btn btn-primary" role="button"> Turn to Principal Co-ordinate Analysis 
→</a>
:::